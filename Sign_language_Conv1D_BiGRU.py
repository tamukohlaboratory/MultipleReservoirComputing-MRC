...
from tensorflow.keras.layers import Dense, Dropout, GRU, Bidirectional, BatchNormalization
...
def ConvBiGRU_model(X_train, y_train,
                X_val, y_val,
                X_test, y_test,
                pathWorkspace,
                learning_rate_param,
                ...
                batch_size,
                output_class,
                model_name="best_model/best_model_convbigru.keras",
                dataUsed=""):
  
    start_time = datetime.now()
    model = Sequential()
    model.add(layers.Masking(mask_value=0, input_shape=(X_train.shape[1], X_train.shape[2])))
    model.add(layers.Conv1D(filters=128, kernel_size=11, activation='relu', padding='causal'))
...
def run_experiment(X_train, y_train,
    ...
    for iteration in range(repeats):
        loss, score, f1, duration_train, duration_test = ConvBiGRU_model( 
                          X_train, y_train,
                          X_val, y_val,
